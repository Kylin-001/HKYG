...
<template>
  <div class="max-w-screen-xl mx-auto px-4 lg:px-8 py-6 space-y-6">
      ...
      <div class="bg-white/90 backdrop-blur-md rounded-2xl border border-primary-50/50 overflow-hidden">
        <div class="p-5 border-b border-primary-50/50">
          <div class="flex items-start justify-between mb-3">
            ...
            <div class="text-right">
              <p class="text-lg font-bold text-gold">
                {{ (canteens[activeCanteen].floors.reduce((s, f) => s + f.rating, 0) / canteens[activeCanteen].floors.length).toFixed(1) }}
              </p>
          ...
          <div class="flex flex-wrap gap-2">
            <span
              v-for="feat in canteens[activeCanteen].features"
              :key="feat"
              class="px-2 py-0.5 rounded-full bg-pine/10 text-pine text-[11px]"
            >{{ feat }}</span>
          </div>
        </div>
        ...
        <div class="divide-y divide-primary-50/50">
          <div
            v-for="floor in canteens[activeCanteen].floors"
            :key="floor.name"
            class="p-4 hover:bg-primary-50/20 transition-colors"
          >
            ...
            <div class="flex flex-wrap gap-1.5 mt-2">
              <span
                ...
                :key="tag"
                class="px-2 py-0.5 rounded-lg bg-gray-100 text-text-secondary text-[11px]"
              >{{ tag }}</span>
            </div>
          </div>
        </div>
    ...
    <section class="bg-info/5 rounded-2xl border border-info/10 p-5">
      <h3 class="text-sm font-bold text-info mb-2">
        💡 用餐小贴士
      </h3>
