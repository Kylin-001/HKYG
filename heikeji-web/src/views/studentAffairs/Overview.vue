...
<template>
  <div class="max-w-screen-xl mx-auto px-4 lg:px-8 py-6">
    ...
    <div v-if="store.loading" class="space-y-6">
      <div class="h-32 rounded-2xl bg-primary-50 animate-pulse"></div>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div v-for="i in 4" :key="i" class="p-4 rounded-2xl bg-primary-50 animate-pulse">
          <div class="w-11 h-11 rounded-xl bg-white/60 mb-3"></div>
          <div class="h-4 w-20 bg-white/40 rounded mb-1"></div>
          <div class="h-3 w-full bg-white/30 rounded"></div>
        </div>
      </div>
      <div class="h-48 rounded-2xl bg-primary-50 animate-pulse"></div>
    </div>

    <template v-else>
    <!-- 待办事项 -->
    <section class="mb-6" v-if="store.pendingTasks.length > 0 || !store.loading">
      <h2 class="text-lg font-bold text-text-primary mb-3 flex items-center gap-2">
        <el-icon :size="20" class="text-warning"><Bell /></el-icon>我的待办
      </h2>
      <div v-if="urgentTasks.length > 0" class="space-y-2 mb-3">
        <div v-for="task in urgentTasks" :key="task.id"
          ...
          <el-icon :size="18" class="text-crimson shrink-0"><Warning /></el-icon>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-text-primary">{{ task.title }}</p>
            <p class="text-xs text-text-tertiary mt-0.5">{{ task.description }}</p>
          </div>
          <span class="px-2 py-0.5 rounded-full bg-crimson text-white text-[10px] font-bold shrink-0">紧急</span>
        </div>
      </div>
      <div v-if="normalTasks.length > 0" class="space-y-2">
        <div v-for="task in normalTasks" :key="task.id"
          @click="router.push(task.actionUrl)"
          ...
          <el-icon :size="18" class="text-warning shrink-0"><Clock /></el-icon>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-text-primary">{{ task.title }}</p>
            <p class="text-xs text-text-tertiary mt-0.5">{{ task.description }}</p>
          </div>
      ...
      <div v-if="store.pendingTasks.length === 0 && !store.loading" class="p-6 rounded-xl bg-pine/5 border border-pine/10 text-center">
        <p class="text-3xl mb-2">✅</p>
        <p class="text-sm font-medium text-pine">暂无待办事项</p>
        <p class="text-xs text-text-tertiary mt-1">所有业务均已处理完毕</p>
      </div>
    </section>
    ...
    <section class="mb-6">
      <h2 class="text-lg font-bold text-text-primary mb-3 flex items-center gap-2">
        <el-icon :size="20" class="text-primary"><Document /></el-icon>快捷服务
      </h2>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div v-for="service in quickServices" :key="service.path"
          ...
          <h4 class="font-semibold text-sm text-text-primary mb-1">{{ service.title }}</h4>
          <p class="text-[11px] text-text-tertiary line-clamp-2 leading-relaxed">{{ service.desc }}</p>
        </div>
      </div>
    </section>
    ...
    <section>
      <div class="flex items-center justify-between mb-3">
        ...
        <button @click="router.push('/student-affairs/policy')" class="text-xs text-text-tertiary hover:text-primary transition-colors flex items-center gap-1">
          查看全部<el-icon><ArrowRight /></el-icon>
        </button>
      </div>
      <div class="bg-white/90 backdrop-blur-md rounded-2xl border border-primary-50/50 divide-y divide-primary-50/50">
        <div v-for="(notice, i) in [
          ...
          <span :class="['px-1.5 py-0.5 rounded text-[9px] font-bold shrink-0',
            notice.tag === '重要' ? 'bg-crimson/10 text-crimson' : notice.tag === '新生' ? 'bg-gold/10 text-gold' : 'bg-info/10 text-info']">{{ notice.tag }}</span>
          <p class="text-sm text-text-primary flex-1 line-clamp-1">{{ notice.title }}</p>
