...
async function handlePay() {
  if (selectedIds.value.length === 0) return ElMessage.warning(t('error.selectItem'))
  try {
    ...
    ElMessage.error(err instanceof Error ? err.message : t('error.payFailed'))
  }
}
...
<template>
  <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-sm text-slate-500">
            {{ new Date().getFullYear() - 1 }}-{{ String(new Date().getFullYear()).slice(2) }} 学年
          </p>
        </div>
      </div>
      <span
        v-if="unpaidCount > 0"
        class="px-4 py-2 rounded-full bg-gradient-to-r from-rose-500 to-rose-600 text-white text-sm font-bold shadow-lg shadow-rose-500/30 animate-pulse"
      >
        {{ unpaidCount }} 项待缴
      </span>
    </div>
    ...
    <section class="relative overflow-hidden bg-white rounded-2xl border border-slate-100 shadow-lg shadow-slate-200/50">
      <!-- 顶部装饰条 -->
      <div class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-500" />
        ...
        <label class="group flex items-center gap-3 cursor-pointer p-3 rounded-xl bg-slate-50 hover:bg-blue-50/50 transition-colors duration-300 border border-transparent hover:border-blue-200">
          <div class="relative">
            <input
              ...
              @change="selectAll"
            >
            <div class="absolute inset-0 rounded-lg bg-blue-600 scale-0 peer-checked:scale-100 transition-transform duration-200 -z-10" />
          </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                ...
                <span
                  v-if="item.status === 'paid'"
                  class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200"
                >
                  已缴
                </span>
                <span
                  v-else-if="item.status === 'overdue'"
                  class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-500 text-white shadow-sm"
                >
                  逾期
                </span>
                <span
                  v-else
                  class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200"
                >
                  待缴
                </span>
              </div>
            ...
            <div class="text-right">
              <p
                ...
                :class="item.status === 'paid' ? 'text-emerald-600' : 'text-blue-600'"
              >
      ...
      <div
        ...
        class="p-6 border-t border-slate-100 bg-gradient-to-r from-slate-50/50 to-transparent"
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <div>
              <p class="text-sm text-slate-500">
                已选 <span class="font-semibold text-slate-900">{{ selectedIds.length }}</span> 项
              </p>
          ...
          <div class="flex items-center gap-4">
            <p class="text-3xl font-bold text-blue-600 tracking-tight">
    ...
    <section class="relative overflow-hidden rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50/50 to-indigo-50/30 p-5">
      <!-- 装饰背景 -->
      <div class="absolute top-0 right-0 w-32 h-32 bg-blue-100/50 rounded-full -translate-y-1/2 translate-x-1/2" />
      ...
      <div class="relative">
        ...
        <ul class="text-sm text-blue-800/70 space-y-2">
          <li class="flex items-start gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-blue-400 mt-2 flex-shrink-0" />
