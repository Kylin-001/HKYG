...
services:
  ...
  # 应用服务
  admin:
    image: heikeji-mall/admin:latest
    container_name: heikeji-admin
    networks: [heikeji-network]
    volumes:
      - ./logs:/app/logs
      - ./.env.prod:/app/.env
...
1. 查询特定服务的错误日志：
   ```
   service_name:"heikeji-admin" AND level:"ERROR"
   ```
...
# 重启服务使配置生效
systemctl restart heikeji-admin
```
...
**排查步骤**：
1. 检查应用日志：`tail -n 200 /logs/heikeji-admin.log`
...
```bash
# 启动应用
systemctl start heikeji-admin

# 停止应用
systemctl stop heikeji-admin

# 重启应用
systemctl restart heikeji-admin

# 查看应用状态
systemctl status heikeji-admin

# 查看应用日志
journalctl -u heikeji-admin -n 100 --no-pager
```
