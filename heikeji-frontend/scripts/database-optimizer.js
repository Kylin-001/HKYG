...
async function generateIndexRecommendations(analyses) {
  ...
  for (const analysis of analyses) {
    const recommendations = databasePerformanceAnalyzer.generateIndexRecommendations(
      analysis.tableName
    )
...
function generateOptimizationSQL(recommendations) {
  ...
  if (groupedRecommendations.new.length > 0) {
    sql += '-- 新建索引\n'
    sql += groupedRecommendations.new
      .map(
        rec =>
  ...
  if (groupedRecommendations.modify.length > 0) {
    sql += '-- 修改索引\n'
    sql += groupedRecommendations.modify
      .map(rec => `ALTER TABLE ${rec.tableName} DROP INDEX ${rec.indexName};`)
      .join('\n')
    sql += '\n'
  }
  ...
  if (groupedRecommendations.drop.length > 0) {
    sql += '-- 删除索引\n'
    sql += groupedRecommendations.drop
      .map(rec => `DROP INDEX ${rec.indexName}; -- ${rec.description}`)
      .join('\n')
    sql += '\n'
  }
...
async function generateReport(analyses, recommendations, sqlResults) {
  ...
  const reportPath = path.join(
    config.outputDir,
    `optimization-report-${new Date().toISOString().split('T')[0]}.md`
  )
  ...
  if (config.indexOptimization.generateSQL) {
    const sqlPath = path.join(
      config.outputDir,
      `optimization-${new Date().toISOString().split('T')[0]}.sql`
    )
