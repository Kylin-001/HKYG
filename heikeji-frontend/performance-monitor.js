...
function saveToHistory(performanceData) {
  ...
  console.log(
    `${config.colors.success}✅ 性能测试结果已添加到历史记录: ${historyFile}${config.colors.reset}`
  ...
  try {
    ...
    if (files.length > config.historyLimit) {
      ...
      filesToDelete.forEach(file => {
        ...
        console.log(
          `${config.colors.info}🗑️  已删除旧的性能历史记录: ${file}${config.colors.reset}`
        )
      })
    }
  } catch (error) {
    console.log(
      `${config.colors.error}❌ 清理性能历史记录时出错: ${error.message}${config.colors.reset}`
    )
  }
}
...
function generatePerformanceTrend() {
  try {
    ...
    console.log(
      `${config.colors.info}  渲染时间: ${trendReport.summary.latest.metrics.renderTime.average.toFixed(2)}ms${config.colors.reset}`
    )
    console.log(
      `${config.colors.info}  API响应时间: ${trendReport.summary.latest.metrics.apiResponseTime.average.toFixed(2)}ms${config.colors.reset}`
    )
    console.log(
      `${config.colors.info}  内存使用: ${trendReport.summary.latest.metrics.memoryUsage.average.toFixed(2)}MB${config.colors.reset}`
    )
    console.log(
      `${config.colors.info}  CPU使用率: ${trendReport.summary.latest.metrics.cpuUsage.average.toFixed(2)}%${config.colors.reset}`

  } catch (error) {
    console.log(
      `${config.colors.error}❌ 生成性能趋势报告时出错: ${error.message}${config.colors.reset}`
    )
  }
}
