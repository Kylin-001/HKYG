...
function copyCoverageReport() {
  ...
  try {
    const coverageData = JSON.parse(fs.readFileSync(`${config.coverageDir}/coverage-summary.json`, 'utf8'));
    ...
    console.log(`${config.colors.success}✅ 覆盖率报告已保存到 ${historyFile}${config.colors.reset}`);
    ...
    console.log(`${config.colors.error}❌ 处理覆盖率报告时出错: ${error.message}${config.colors.reset}`);
  }
}
...
function cleanupOldHistory() {
  try {
    ...
    console.log(`${config.colors.error}❌ 清理历史记录时出错: ${error.message}${config.colors.reset}`);
  }
}
...
function generateTrendReport() {
  try {
    ...
    console.log(`${config.colors.success}✅ 覆盖率趋势报告已生成: ${trendFile}${config.colors.reset}`);
    
    // 打印趋势摘要
    console.log(`${config.colors.info}📈 覆盖率趋势摘要:${config.colors.reset}`);
    console.log(`${config.colors.info}  最新覆盖率: ${trendReport.summary.latest.coverage.lines.pct}%${config.colors.reset}`);
    console.log(`${config.colors.info}  平均覆盖率: ${trendReport.summary.average.lines.pct}%${config.colors.reset}`);
    console.log(`${config.colors.info}  趋势: ${trendReport.summary.trend.lines}${config.colors.reset}`);
    
  } catch (error) {
    console.log(`${config.colors.error}❌ 生成趋势报告时出错: ${error.message}${config.colors.reset}`);
  }
}
