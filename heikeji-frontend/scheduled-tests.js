...
function runScheduledTest(schedule) {
  ...
  const logFile = path.join(
    config.logDir,
    `${schedule.name}-${dateStr}_${timeStr.replace(/:/g, '-')}.log`
...
function sendNotification(schedule, status, message) {
  ...
  console.log(
    `${config.colors.info}  状态: ${status === 'success' ? '成功' : '失败'}${config.colors.reset}`
  )
...
function checkSchedules() {
  ...
  config.schedules.forEach(schedule => {
    if (schedule.enabled) {
      console.log(
        `${config.colors.info}  ✅ ${schedule.name} - ${schedule.description}${config.colors.reset}`
      )
      console.log(`${config.colors.info}    计划: ${schedule.schedule}${config.colors.reset}`)
    } else {
      console.log(
        `${config.colors.info}  ❌ ${schedule.name} - ${schedule.description} (已禁用)${config.colors.reset}`
      )
    }
  })
  ...
  case 'run':
    if (scheduleName) {
      ...
      if (schedule) {
        ...
        console.log(
          `${config.colors.error}❌ 未找到名为 ${scheduleName} 的定时测试任务${config.colors.reset}`
        )
      }
    } else {
      executeAllSchedules()
