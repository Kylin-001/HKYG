    ...
    const resourceObserver = new PerformanceObserver((list) => {
      ...
      entries.forEach(entry => {
        if (entry.transferSize) {
          ...
          compressedSize += (entry as any).encodedBodySize || (entry as any).transferSize
        }
      })
...
class PerformanceOptimizer {
  // 检测性能问题并提供建议
  analyze(metrics: PerformanceMetrics): Array<{ type: string; message: string; priority: 'high' | 'medium' | 'low' }> {
    const suggestions = []
    ...
    // 检查资源大小
    if (metrics.totalSize > 1024 * 1024 * 2) { // 2MB
      suggestions.push({
        type: 'size',
