...
class CacheMonitor {
  ...
  private checkAlerts(metrics: CacheMetrics): void {
    ...
    const memoryUsageThreshold = parseFloat(
      import.meta.env.VITE_CACHE_ALERT_MEMORY_USAGE_ABOVE || '0.9'
    )
    ...
    if (metrics.memoryUsage > 0) {
      ...
      if (usageRate > memoryUsageThreshold) {
        console.warn(
          `内存使用告警: ${(usageRate * 100).toFixed(2)}% > ${(memoryUsageThreshold * 100).toFixed(2)}%`
        )
...
class CacheAnalyzer {
  ...
  analyzePerformance(period: 'hour' | 'day' | 'week' = 'day'): CacheAnalysis {
    ...
    // 计算平均值
    const avgHitRate =
      relevantMetrics.reduce((sum, m) => sum + m.hitRate, 0) / relevantMetrics.length
    const avgMemoryUsage =
      relevantMetrics.reduce((sum, m) => sum + m.memoryUsage, 0) / relevantMetrics.length
    const avgKeyCount =
      relevantMetrics.reduce((sum, m) => sum + m.totalKeys, 0) / relevantMetrics.length
  ...
  private generateRecommendations(
    ...
    const maxMemory = parseFloat(import.meta.env.VITE_CACHE_MAX_MEMORY || '10485760')
    const memoryUsageThreshold = parseFloat(
      import.meta.env.VITE_CACHE_MEMORY_USAGE_THRESHOLD || '0.8'
    )
...
\`\`\`
  ...
  // 生成趋势表格
  private static generateTrendTable(
...
export { CacheMonitor, CacheAnalyzer, CacheReportGenerator }
