...
import {
  ...
  databaseOptimizationGenerator,
  DatabaseOptimizationGenerator,
} from '@/utils/database-performance'
...
const mockQueryResults = {
  ...
  'SELECT * FROM user WHERE status = 1': {
    ...
    recommendations: ['查询使用了索引，但执行时间过长', '考虑添加复合索引'],
  },
  ...
  'SELECT * FROM order o JOIN order_item oi ON o.id = oi.order_id JOIN user u ON o.user_id = u.id WHERE o.status = 1 AND o.create_time > "2023-01-01"':
    {
      ...
      recommendations: ['查询使用了合适的索引', '查询效率良好'],
    },
}
...
describe('Database Performance Analyzer', () => {
  ...
  describe('Table Analysis', () => {
    it('should analyze user table', async () => {
      // 模拟表分析
      vi.spyOn(databasePerformanceAnalyzer, 'analyzeTable').mockResolvedValue(
        mockTableAnalyses.user
      )
  ...
  describe('Database Optimization Generator', () => {
    it('should generate optimization SQL', () => {
      ...
      const sql = DatabaseOptimizationGenerator.generateIndexOptimizationSQL(recommendations)

      expect(sql).toContain(
        "ALTER TABLE user ADD INDEX idx_user_email (email) USING BTREE COMMENT '用户邮箱唯一索引'"
      )
      expect(sql).toContain(
        "ALTER TABLE user ADD INDEX idx_user_phone (phone) USING BTREE COMMENT '用户手机号唯一索引'"
      )
  ...
  describe('Database Performance Monitor', () => {
    ...
    it('should trigger alert for low cache hit rate', () => {
      ...
      const stats = monitor.getQueryStats()
      const hitRate = stats.filter((stat: any) => stat.hitCache).length / stats.length
