  ...
  async mget<T>(keys: string[]): Promise<Array<T | null>> {
    try {
      ...
      if ('mget' in this.adapter) {
        const values = await (this.adapter as any).mget(keys)
...
// 缓存装饰器
export function Cacheable(options?: { ttl?: number; tags?: string[]; key?: string }) {
...
export { CacheService }
export type { CacheMetadata, CacheServiceConfig }
