...
describe('API Performance Tests', () => {
  ...
  describe('Product API Performance', () => {
    it('should handle concurrent product list requests efficiently', async () => {
      const apiCall = () => productApi.getProductList({ pageNum: 1, pageSize: 10 })
    ...
    it('should handle large data requests efficiently', async () => {
      const apiCall = () => productApi.getProductList({ pageNum: 1, pageSize: 100 })
    ...
    it('should maintain consistent response times', async () => {
      ...
      const variance =
        performance1.times.reduce((sum, time) => {
          const diff = time - performance1.avgTime
          return sum + diff * diff
        }, 0) / performance1.times.length
  ...
  describe('Order API Performance', () => {
    it('should handle order list pagination efficiently', async () => {
      ...
      const maxVariance = Math.max(...avgTimes.map(time => time * time))
