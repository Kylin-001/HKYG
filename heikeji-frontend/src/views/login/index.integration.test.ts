...
describe('登录流程集成测试', () => {
  ...
  it('应该成功登录并跳转到仪表盘', async () => {
    ...
    await passwordInput.setValue(testUserData.password)
  ...
  it('应该在登录失败时显示错误信息', async () => {
    const loginMock = vi.fn().mockRejectedValue(new Error('用户名或密码错误'))
  ...
  it('应该在登录成功后保存用户信息', async () => {
    ...
    expect(userStore.token).toBe(testUserData.token)
  ...
  it('应该支持验证码登录', async () => {
    ...
    const { login, getCaptcha } = await import('@/api/login')
    const { setToken } = await import('@/utils/auth')
